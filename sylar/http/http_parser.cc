...
static int on_request_message_complete_cb(http_parser *p) {
    ...
    // 返回非0，让http-parser在第一条消息结束处立即停止，
    // 避免keep-alive时把缓冲区中下一条消息也一并解析掉（丢失粘包边界）
    return 1;
}
...
// 这段代码是 nodejs/http-parser 的回调函数注册表（callback table），用于告诉 HTTP 解析器：
// 解析 HTTP 请求的不同阶段，应该调用哪个函数。例如：
// .on_message_begin解析开始时，要执行回调函数 on_request_message_begin_cb,
// .on_url解析url时，要执行回调函数 on_request_url_cb
// ......
...
size_t HttpRequestParser::execute(char *data, size_t len) {
    // 进行解析，返回实际解析处理了多少个的字节
    // m_parser：解析过程状态对象
    // s_request_settings：解析阶段对应的各个处理函数
    // data、len：需要解析的数据 和 数据长度
    size_t nparsed = http_parser_execute(&m_parser, &s_request_settings, data, len);
    if (m_parser.upgrade) {
        //表明后续将使用一个新协议（可能是更新的http，甚至不是http），故需要处理。（本系统中暂时不处理）
        SYLAR_LOG_DEBUG(g_logger) << "found upgrade, ignore";
        setError(HPE_UNKNOWN);
    } else if (m_parser.http_errno != 0
               && m_parser.http_errno != HPE_CB_message_complete) {
        //解析过程有错误（HPE_CB_message_complete 表示第一条消息已解析完，属正常停止）
        ...
        if (nparsed < len) {//目前只解析了一部分，将未解析的部分前移，待下次解析。（流式解析）
