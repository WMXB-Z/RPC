...
public:
    ...
    /**
     * @brief 添加servlet。并且这里的servlet类型未知，可根据T模版参数构建（间接调用addServletCreator(const std::string& uri, IServletCreator::ptr creator)）
    ...
    /**
     * @brief 添加模糊匹配servlet，这里的servlet类型已知为FunctionServlet
     * 间接调用addGlobServlet(const std::string& uri, Servlet::ptr slt);
     * @param[in] uri uri 模糊匹配 /sylar_*
     * @param[in] cb FunctionServlet回调函数
     */
    ...
    /**
     * @brief 添加模糊匹配servlet。并且这里的servlet类型未知，可根据T模版参数构建
     * 间接调用addGlobServletCreator(const std::string& uri, IServletCreator::ptr creator);
     * @param[in] uri uri 模糊匹配 /sylar_*
     */
    template<class T>
