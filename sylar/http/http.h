...
/**
 * @brief HTTP方法枚举
 * 即HTTP协议支持的一些列操作：GET、PUT、DELETE......
 */
...
/**
 * @brief 将字符串方法名转成HTTP方法枚举（本系统未使用）
 * @param[in] m HTTP方法
 * @return HTTP方法枚举
 */
...
/**
 * @brief 将字符串指针转换成HTTP方法枚举（本系统未使用）
 * @param[in] m 字符串方法枚举
 * @return HTTP方法枚举
 */
...
public:
    ...
    /**
     * @brief 检查并获取HTTP请求的Cookie参数（本系统未使用）
     * @tparam T 转换类型
     * @param[in] key 关键字
     * @param[out] val 返回值
     * @param[in] def 默认值
    ...
    /**
     * @brief 获取HTTP请求的Cookie参数（本系统未使用）
     * @tparam T 转换类型
     * @param[in] key 关键字
     * @param[in] def 默认值
     * @return 如果存在且转换成功返回对应的值,否则返回def
