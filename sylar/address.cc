...
bool Address::Lookup(std::vector<Address::ptr> &result, const std::string &host,
                     int family, int type, int protocol) {
    addrinfo hints, *results, *next;    //hints是地址检索时的限制条件（限制是什么地址族family,socket是什么类型,socket使用什么协议），检索结果放入results
    hints.ai_flags     = 0;
    hints.ai_family    = family;
    hints.ai_socktype  = type; //未指定的条件下，type在接口定义时默认设置为0.(这表示查询所有sock类型！)  
    hints.ai_protocol  = protocol;  //同样，未指定的条件下，默认为0(表示让内核根据 family 和 type 自动选择协议。)
    hints.ai_addrlen   = 0;
    hints.ai_canonname = NULL;
    hints.ai_addr      = NULL;
    hints.ai_next      = NULL;

    // 拆分节点（IP/addr） 与 服务（port）
    std::string node;
    const char *service = NULL;

    //检查 ipv6address serivce
    if (!host.empty() && host[0] == '[') {  //IPv6的地址要求 [IPv6]:port 的形式，这是因为IPV6允许::缩写0，所以不方便直接使用':'定位
        const char *endipv6 = (const char *)memchr(host.c_str() + 1, ']', host.size() - 1); //定位']'的位置
        if (endipv6) {
    ...
    //检查 node serivce
    if (node.empty()) { // 普通 IPv4/域名处理
        service = (const char *)memchr(host.c_str(), ':', host.size()); //定位 ':'的位置
        if (service) {
            if (!memchr(service + 1, ':', host.c_str() + host.size() - service - 1)) {
                node = host.substr(0, service - host.c_str());  //域名或IP
                ++service;//port号
            }
        }
    }
    ...
    int error = getaddrinfo(node.c_str(), service, &hints, &results);   //Linux 提供的地址解析函数。返回的查询结果results是一个链表
    if (error) {
    ...
    freeaddrinfo(results);  //释放由getaddrinfo申请的资源
    return !result.empty();
}
