...
public:
     ...
     * 只要结束就是TERM状态。也不区别HOLD状态，协程只要未结束也非运行态，那就是READY状态。因此，系统设置，对于协程的重置操作，只能对TREM状态下的协程执行
     */
    enum State {
        /// 就绪态，刚创建或者yield之后的状态
        READY,
...
private:
    /**
     * @brief 构造函数
     * @attention 无参构造函数只用于创建线程的第一个协程，也就是线程主函数对应的主协程，
...
public:
    ...
    /**
     * @brief 将当前协程切到到执行状态（当前执行流从主协程切换到某个子协程执行。）
     * @details 当前协程和正在运行的协程进行交换，前者状态变为RUNNING，后者状态变为READY
     */
    void resume();

    /**
     * @brief 当前协程让出执行权（当前子协程主动让出 执行权，切回主协程。）
...
public:
    /**
     * @brief 将本协程设置当前正在运行的协程，即设置线程局部变量t_fiber的值
     */
...
private:
    // !每个协程对象拥有自己独立的执行上下文（context,包含寄存器内容）和独立的协程栈（stack,包含函数栈、局部变量等信息），
    // !这两部分共同描述了“这个协程暂停时的完整执行状态”。不同协程之间不会共享这些执行现场。
    ...
    /// 本协程是否参与调度器调度
    // （这个比较关键，如果为true，则表示本协程是一个任务执行协程，它只能和调度协程做切换，而不是和任务添加协程做切换）
