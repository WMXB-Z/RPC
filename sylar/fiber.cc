...
Fiber::ptr Fiber::GetThis() {
    ...
    Fiber::ptr main_fiber(new Fiber);   //当前线程没有正在运行的协程，则需要先创建主协程
...
Fiber::~Fiber() {
    ...
    if (m_stack) {
        ...
        Fiber *cur = t_fiber;
        if (cur == this) { // 当前协程就是自己，则需要将当前协程置空
            SetThis(nullptr);
        }
    }
}
...
void Fiber::resume() {
    ...
    // 如果协程参与调度器调度，则说明这是一个任务执行协程，
    // 那么应该和调度器的调度协程进行swap，而不是线程的主协程（即任务添加协程）
    // swapcontext(old, new)：将当前的上下文信息保存至old，并从new中恢复上下文并运行
    if (m_runInScheduler) {
...
void Fiber::yield() {
    ...
    if (m_state != TERM) {
        m_state = READY;    //由running-->ready
    }

    // 如果本协程参与调度器调度，说明这是一个"任务执行协程"，那么应该和调度器的"调度协程"进行swap，而不是线程主协程"任务添加协程"进行swap
    if (m_runInScheduler) {
...
void Fiber::MainFunc() {
    ...
    cur.reset();    // 手动让t_fiber的引用计数减1，之所以主动释放，是因为下面调用yield()回到主协程后不再切换回来，该函数不会正常结束！
    raw_ptr->yield();   //切换回主协程
}
