 ...
 * @file env.cc
 * @brief 环境变量管理接口实现
 * @todo 命令行参数解析应该用getopt系列接口实现，以支持选项合并和--开头的长选项。（目前，自定义的解析模块不支持）
 */
#include "env.h"
#include "sylar/log.h"
#include <string.h>
...
bool Env::init(int argc, char **argv) {
    char link[1024] = {0};  //保存/proc路径(虚拟文件路径)
    char path[1024] = {0};  //保存真实路径
    sprintf(link, "/proc/%d/exe", getpid());   //得到一个link路径。例如：   /path/123/exe
    readlink(link, path, sizeof(path));//通过读这个link路径，得到一个可执行文件的真实路径。即/path/123/exe-->/home/project/bin/server
    m_exe = path;

    auto pos = m_exe.find_last_of("/");
    m_cwd    = m_exe.substr(0, pos) + "/";  //得到文件目录路径
    ...
    for (int i = 1; i < argc; ++i) {
        if (argv[i][0] == '-') {
            if (strlen(argv[i]) > 1) {
                if (now_key) {
                    add(now_key, "");   //上轮没有置空，说明上一个参数没有值，故默认设置为nullptr
                }
                now_key = argv[i] + 1;
            } else {
