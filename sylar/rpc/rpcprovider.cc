...
// 初始化PrcProvider，包括创建 TCP Server
...
void RpcProvider::InnerStart() {
    ...
    // 注册到 zk 上， rpc client可以从zkserver上发现服务
    m_zkcli->start();
...
/**
 * @brief 从client读取固定长度的数据（用于接收RPC协议包）
...
void RpcProvider::InnerHandleClient(sylar::Socket::ptr client) {
    ...
    while (client->isConnected()) {
        ...
        if (all_size > recv_buf.size()) {   //说明包不完整
        ...
        // 设置Closure对象（相当于一个回调处理）
        google::protobuf::Closure* done = 
            google::protobuf::NewCallback<sylar::rpc::RpcProvider, sylar::Socket::ptr, 
                                        google::protobuf::Message*> (this, &RpcProvider::SendRpcResopnse, client, response);
        service->CallMethod(method, nullptr, request, response, done);  //RPC服务的正式调用
    }
    client->close();
}
