...
bool Socket::bind(const Address::ptr addr) {
    m_localAddress = addr;
    if (!isValid()) {
        newSock();  //如果Socket中拥有的sock实际上未创建，则先创建
    ...
    //不是unix套接字，说明这是一个网络套接字
    if (::bind(m_sock, addr->getAddr(), addr->getAddrLen())) {  //进行sock + 地址addr的绑定
        SYLAR_LOG_ERROR(g_logger) << "bind error errrno=" << errno
                                  << " errstr=" << strerror(errno);
        return false;
    }
    getLocalAddress();  //这里主要是为了将socket中地址设置为绑定好了的地址addr
    return true;
}
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
    ...
    if (timeout_ms == (uint64_t)-1) {//未指定timeout_ms，使用系统默认的timeout_ms
...
Address::ptr Socket::getLocalAddress() {
    ...
    socklen_t addrlen = result->getAddrLen();
    if (getsockname(m_sock, result->getAddr(), &addrlen)) { //获得m_sock绑定的本地地址，并写入result中
